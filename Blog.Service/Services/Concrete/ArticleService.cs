	...
	public class ArticleService : IArticleService
	{
		...
		public async Task<ArticleListDTO> GetAllByPagesAsync(Guid? CategoryId, int currentPage = 1, int pageSize = 3, bool isAscending = false)
		{
			...
			var articles = CategoryId == null
				? await _unitOfWork.GetRepository<Article>().GetAllAsync(x => !x.IsDeleted, a => a.Category, i => i.Image,u=> u.User)
				: await _unitOfWork.GetRepository<Article>().GetAllAsync(x => x.CategoryId == CategoryId && !x.IsDeleted, a => a.Category, i => i.Image,u=> u.User);
		...
		{
			var article = await _unitOfWork.GetRepository<Article>().GetAsync(x => !x.IsDeleted && x.Id == id, c => c.Category, c => c.Image ,u=> u.User);
		...
		public async Task<ArticleListDTO> SearchAsync(string keyword, int currentPage = 1, int pageSize = 3, bool isAscending = false)
		{
			...
			var articles = await _unitOfWork.GetRepository<Article>().GetAllAsync(x => !x.IsDeleted && (x.Title.Contains(keyword) || x.Content.Contains(keyword) || x.Category.Name.Contains(keyword))
